...
pps_raw = pd.read_csv('/Users/madeleineseitz/Desktop/thesis/ABCD_Project/csvs/PQ-BC/pps01.csv')
# %%
...
pps_all = pps_all.drop(columns= ['collection_id', 'pps01_id', 'dataset_id', 'subjectkey', 'src_subject_id', 'interview_date', 
                                 'interview_age', 'sex', 'visit', 'collection_title'])
...
pps_sums.to_csv('/Users/madeleineseitz/Desktop/thesis/ABCD_Project/csvs/PQ-BC/pps01_sums.csv')
#shorten df to contain only sums and subject identification info
pps_short = pps_sums[['subjectkey', 'sex', 'visit', 'PQBC_total_endorsed', 'PQBC_total_bother', 'PQBC_total_bother_intensity', 'mean_intensity_per_bother_Q']]
# %%
...
# %%
#add high risk "group" column to pps_short df and pps_2 df
HR_list = []
for key in risk_2:
    if risk_2[key] == 'HR':
        HR_list.append(key)
        len(HR_list)
pps_short['group'] = 0
for row in pps_short.index:
    if pps_short['subjectkey'][row] in HR_list:
        pps_short['group'][row] = 1
pps_2['group'] = 0
for row in pps_2.index:
    if pps_2['subjectkey'][row] in HR_list:
        pps_2['group'][row] = 1
pps_2['group'].sum()
pps_short = pps_short.drop([0])
#save df to file
pps_short.to_csv('/Users/madeleineseitz/Desktop/thesis/ABCD_Project/csvs/PQ-BC/PQBC_4.csv')
# %%
